...
public class EmployeePersonnelNumber
{
    ...
    private static void Validate(string personnelNumber)
    {
        ...
        if(delimitersCount != 1)
            throw new ArgumentException("The personnel number must contain only one delimiter");
    }
    ...
    private void Parse(string personnelNumber)
    {
        ...
        if(!internalCompanyNumberParsed)
            throw new ArgumentException("Couldn't parse internal company number");
        if(!hiringYearParsed)
            throw new ArgumentException("Couldn't parse hiring year");
